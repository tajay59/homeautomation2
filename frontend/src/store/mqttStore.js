    ...
    */ 
    ...
    const onMessageArrived = (response) => {
           ...
           try {
            ...
            console.log(`Received new MQTT message`);  
           } catch (error) {
    ...
    const connect = ()=> {
        ...
        var options   = { timeout: 3, onSuccess: onSuccess, onFailure: onFailure, invocationContext: {"host":host.value,"port": port.value }, useSSL: true, reconnect: true, uris:[`ws://${host.value}:${port.value}/mqtt`] }; 
